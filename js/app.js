(() => {
    ...
    (() => {
        ...
        let picker;
        if (picker) {
            picker = datepicker_min("[data-datepicker]", {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".rocks__slider")) new core(".rocks__slider", {
                modules: [ Navigation, Pagination, Lazy, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                on: {
                    init: function() {
                        const control = document.querySelectorAll(".rocks__bullets .swiper-pagination-bullet");
                        control.forEach(((el, index) => {
                            let num = index < 10 ? `0` : "";
                            el.innerHTML = `${num}${index + 1}`;
                        }));
        ...
        if (document.querySelector(".video-module")) document.addEventListener("watcherCallback", (function(e) {
            const entry = e.detail.entry;
            const targetElement = entry.target;
            if ("video" === targetElement.dataset.watch) if (entry.isIntersecting) targetElement.querySelector("video").play(); else targetElement.querySelector("video").pause();
        }));
        window["FLS"] = true;
        isWebp();
        addTouchClass();
